# -*- coding: utf-8 -*-
"""Build FMS Academic Article DOCX from markdown."""
...
INPUT_MD = ROOT / "FMS-Academic-Article-AI-Diplomacy-Peace.md"
OUTPUT_DOCX = ROOT / "FMS-Academic-Article-AI-Diplomacy-Peace.docx"
...
def add_figure_placeholder(doc, caption, figure_num):
    p = doc.add_paragraph()
    p.alignment = WD_ALIGN_PARAGRAPH.CENTER
    set_run_font(
        p.add_run(f"[Figure {figure_num}: See Mermaid diagram in source markdown]"),
        italic=True,
        size=Pt(10),
    )
    cp = doc.add_paragraph(caption or f"Figure {figure_num}")
    cp.style = "Caption"
    cp.alignment = WD_ALIGN_PARAGRAPH.CENTER
    stats["figures"] += 1
...
def add_title_page(doc):
    ...
    set_run_font(
        t.add_run(
            ...
            "A Human-Centered Framework for Preventive Diplomacy and Global Governance"
        ),
        size=Pt(14),
        bold=True,
    )
    ...
    while i < len(lines):
        ...
        if line.strip().startswith("```"):
            if line.strip().startswith("```mermaid"):
                ...
                i += 1
                figure_count += 1
                add_figure_placeholder(doc, pending_figure_caption, figure_count)
                pending_figure_caption = None
                continue
            i += 1
            while i < len(lines) and not lines[i].strip().startswith("```"):
